...
Object[][] data;
	...
	private int countDistinctTuples(){
		...
		for(int i=0;i<getNumberOfExamples()-1;i++){
			trovato = false;
			int j=i+1;
			while(j<getNumberOfExamples() && trovato==false){
				if(compare(i,j)){
					DistinctTuples--;
					trovato=true;
				}
	...
	public int[] sampling(int k) throws OutOfRangeSampleSize{
		do{
			...
			{
				throw new OutOfRangeSampleSize("il numero k di cluster inserito da tastiera è maggiore maggiore rispetto al numero di centroidi generabili dall'insieme di transazioni");
			}
		}while(k<=0 || k>distinctTuple);
